...
public class MeshSmoother {
	...
	static double tooDistantRatio = 4.0;
	 ...
	 */
	public static boolean smooth(Mesh m, int mad, double ideal) {
		...
		for (int j = 0; j < mesh.getNumTriangles(); j++) {
			...
			//System.out.println("  smoothing " + j + " [" + v[0].id + " " + v[1].id + " " +v[2].id + " ");
			...
			for (int k = 0; k < 3; k++) {
				...
				if(difflen > idist && difflen < (idist * tooDistantRatio)) continue;
				if(difflen < tooSmall) {
				...
				if(difflen > (idist * tooDistantRatio)) {
					movex[v[k].getId()] -= diff[0];
					movey[v[k].getId()] -= diff[1];
					movex[v[k1].getId()] += diff[0];
					movey[v[k1].getId()] += diff[1];
