...
public class MinimizerBarnesHut {
    ...
    private final List<FdNode> nodes;
	...
	private final Map<FdNode,Collection<FdEdge>> attrEdges;
     ...
     *   (e.g. 0.01 for unweighted graphs) otherwise.
	 */
	public MinimizerBarnesHut( 
            final Collection<FdNode> nodes, final Collection<FdEdge> attrEdges, 
            final double repuExponent, final double attrExponent, final double gravFactor) {
        this.nodes = new ArrayList<FdNode>(nodes);
        this.attrEdges = new HashMap<FdNode,Collection<FdEdge>>();
        for (FdNode node : nodes) this.attrEdges.put(node, new ArrayList<FdEdge>());
        for (FdEdge edge : attrEdges) {
	 ...
	 */
	public void minimizeEnergy(final Map<FdNode,double[]> positions, final int nrIterations) {
		if (nodes.size() <= 1) return;
        this.positions = positions;

		repuFactor = computeRepuFactor();
		...
		for (FdNode node : nodes) energySum += getEnergy(node, octTree);
	 ...
	 */
    private double computeRepuFactor() {
        double attrSum = 0.0;
        for (FdNode node : nodes) {
            for (FdEdge edge : attrEdges.get(node)) {
                attrSum += edge.weight;
            }
        }
		double repuSum = 0.0;
		for (FdNode node : nodes) repuSum += node.weight;
		if (repuSum > 0.0 && attrSum > 0.0) {
			return attrSum / repuSum / repuSum
				* Math.pow(repuSum, 0.5*(attrExponent-repuExponent));
		}
		return 1.0;
    }
	 ...
	 */
	private double getRepulsionEnergy(final FdNode node, final OctTree tree) {
	 ...
	 */
	private double getAttractionEnergy(final FdNode node) {
		double energy = 0.0;
	 ...
	 */
	private double getGravitationEnergy(final FdNode node) {
        final double dist = getDist(positions.get(node), baryCenter);
        return gravFactor * repuFactor * node.weight * Math.pow(dist, attrExponent) / attrExponent;
	}
	 ...
	 * @return total energy of the specified node
 	 */
    private double getEnergy(final FdNode node, final OctTree octTree) {
	 ...
	 */
	private double addRepulsionDir(final FdNode node, final OctTree tree, final double[] dir) {
        if (tree == null || tree.node == node) return 0.0;
        if (node.weight == 0.0) return 0.0; 
	 ...
	 */
	private double addAttractionDir(final FdNode node, final double[] dir) {
		double dir2 = 0.0;
        final double[] position = positions.get(node);
		for (FdEdge edge : attrEdges.get(node)) {
	 ...
	 */
	private double addGravitationDir(final FdNode node, final double[] dir) {
        final double[] position = positions.get(node);
	 ...
	 *               (output parameter)
	 */
    private void getDirection(final FdNode node, final OctTree octTree, final double[] dir) {
	...
	private OctTree buildOctTree() {
		...
		for (FdNode node : nodes) result.addNode(node, positions.get(node), 0);
		return result;
	}
	...
	private void printStatistics(final OctTree octTree) {
		...
		for (FdNode node : nodes) meanAttrEnergy += getAttractionEnergy(node);
		meanAttrEnergy = Math.pow(meanAttrEnergy * attrExponent / attrSum, 1.0 / attrExponent); 
		...
		for (FdNode node : nodes) meanRepuEnergy += getRepulsionEnergy(node, octTree);
	...
	static class OctTree {
		 ...
		 */
		public OctTree(FdNode node, double[] position, double[] minPos, double[] maxPos) {
			this.node = node;
		 ...
		 */
		public void addNode(FdNode newNode, double[] newPos, int depth) {
		 ...
		 * @param depth    depth of this tree node in the octtree
		 */
		protected void addNode2(FdNode newNode, double[] newPos, int depth) {
            if (depth == MAX_DEPTH) {
		 ...
		 */
        public void removeNode(FdNode oldNode, double[] oldPos, int depth) {
