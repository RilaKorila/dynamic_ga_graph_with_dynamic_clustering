 ...
 * While <code>MinimizerBarnesHut</code> is faster,
 * <code>MinimizerClassic</code>
...
public class LinLogLayout {
	 ...
	 *
	 */
	public static Map<String, Map<String, Double>> constructGraph(Vector<InputEdge> edgelist) {
		Map<String, Map<String, Double>> result = new HashMap<String, Map<String, Double>>();
	...
	public static Map<String, FdNode> makeNodes(Map<String, Map<String, Double>> graph) {
		// TreeMapを使用してキーを自然順序（数値順）でソート
		Map<String, FdNode> result = new TreeMap<>((a, b) -> {
	...
	public static Map<FdNode, double[]> makeInitialPositions(List<FdNode> nodes, boolean is3d) {
		...
		for (int i = 0; i < nodes.size(); i++) {
			...
			if (initialpos != null && i < initialpos.size()) {
				...
				// counterがnode番号に対応するので、initialposが取れない場合に、
				// 2次元の場合には、(counter x 0.5921, counter x 0.4129)が返るようになっている
				// 掛け算があることで、y = x の直線上からnodeを少しずつずらしている
				System.out.println("initialposをgetできなかったので、ダミーデータを付与");
				position = new double[] { myRandom(counter, 0) - 0.5,
	...
	public static void setFixNodes(
			Map<String, FdNode> nameToNode, Map<FdNode, double[]> nodeToPosition, Vector<OutputNode> fixlist) {
	...
	private static void vectorizeNodes(Map<FdNode, double[]> nodeToPosition,
			Map<FdNode, Integer> nodeToCluster, Vector<OutputNode> nodelist) {
	...
	public static void exec(Vector<InputEdge> edgelist, Vector<OutputNode> nodelist, Vector<OutputNode> fixlist) {
		...
		Map<FdNode, Integer> nodeToCluster = new OptimizerModularity().execute(nodes, edges, true, false);
		vectorizeNodes(nodeToPosition, nodeToCluster, nodelist);
		// writePositions(nodeToPosition, nodeToCluster, args[2]);
	}

	public static void execWithoutForceDirected(Vector<InputEdge> edgelist, Vector<OutputNode> nodelist,
			Vector<OutputNode> fixlist) {

		// グラフ構造の構築（ノードベースの新しいメソッドを使用）
		Map<String, Map<String, Double>> graph = constructGraphBasedOnNodes(edgelist, nodelist);
		graph = makeSymmetricGraphBasedOnNodes(graph);
		...
		// initial positionの設定: この時に initialposから取得するように変更している
		Map<FdNode, double[]> nodeToPosition = makeInitialPositions(nodes, false);

		// 固定ノードの設定 fixlistがnullの場合は、固定ノードの設定を行わない
		setFixNodes(nameToNode, nodeToPosition, fixlist);

		Map<FdNode, Integer> nodeToCluster = loadClusterFromCsv(nodes);
	 ...
	 * 
	 * @param args number of dimensions, name of the input file and of the output
	 *             file.
	 *             If <code>args.length != 3</code>, the method outputs a help
	 *             message.
	 */
	/*
	 ...
	 * List<FdNode> nodes = new ArrayList<FdNode>(nameToNode.values());
	 * List<FdEdge> edges = makeEdges(graph,nameToNode);
	 * Map<FdNode,double[]> nodeToPosition = makeInitialPositions(nodes,
	 * args[0].equals("3"));
	 * // see class MinimizerBarnesHut for a description of the parameters
	 * new MinimizerBarnesHut(nodes, edges, 0.0, 1.0,
	 * 0.01).minimizeEnergy(nodeToPosition, 100);
	 * // see class OptimizerModularity for a description of the parameters
	...
	/**
	 * CSVファイルからクラスタ情報を読み込んでnodeToClusterを作成
	 * 
	 * @param nodes FdNodeのリスト
	 * @return ノードとクラスタIDのマッピング
	 */
	public static Map<FdNode, Integer> loadClusterFromCsv(List<FdNode> nodes) {
		Map<FdNode, Integer> nodeToCluster = new HashMap<>();
		String filepath = ResourceFile.COMMUNITY_CSV.path();
		...
		try (BufferedReader br = new BufferedReader(new FileReader(filepath))) {
			...
			while ((line = br.readLine()) != null) {
				// カンマで分割してnode_idのリストを取得
				String[] nodeIds = line.split(",");

				// 各node_idに対してクラスタIDを割り当て
				for (String nodeId : nodeIds) {
					if (!nodeId.trim().isEmpty()) {
						// FdNodeのnameとnode_idを照合
						for (FdNode node : nodes) {
							if (node.name.equals(nodeId.trim())) {
			...
			// クラスタが割り当てられなかったノードにはデフォルト値を設定
			for (FdNode node : nodes) {
				if (!nodeToCluster.containsKey(node)) {
					nodeToCluster.put(node, -1); // または0など適切なデフォルト値
				}
			}

		} catch (IOException e) {
			System.err.println("Error reading cluster file: " + e.getMessage());
			e.printStackTrace();
			// エラー時はすべてのノードに同じクラスタIDを割り当て
			for (FdNode node : nodes) {
				nodeToCluster.put(node, 0);
			}
		}
	...
	/**
	 * ノードリストに基づいてグラフを構築する
	 * 孤立ノードも含めた全てのノードを考慮する
	 * 
	 * @param edgelist エッジのリスト
	 * @param nodelist ノードのリスト
	 * @return グラフ構造（ノードIDから接続先ノードとその重みへのマッピング）
	 */
	public static Map<String, Map<String, Double>> constructGraphBasedOnNodes(Vector<InputEdge> edgelist,
			Vector<OutputNode> nodelist) {
		Map<String, Map<String, Double>> result = new HashMap<String, Map<String, Double>>();

		// まず全てのノードに対してマップを初期化
		for (OutputNode node : nodelist) {
			String nodeId = Integer.toString(node.id);
			result.put(nodeId, new HashMap<String, Double>());
		}

		// エッジ情報を追加
		for (InputEdge edge : edgelist) {
			String source = Integer.toString(edge.node1);
			String target = Integer.toString(edge.node2);
			double weight = edge.weight;
			result.get(source).put(target, weight);
	...
	/**
	 * 全てのノードペアに対して対称性を確保したグラフを生成する
	 * 孤立ノードも含めて、全てのノードペアを考慮する
	 * 
	 * @param graph 元のグラフ
	 * @return 対称化されたグラフ
	 */
	public static Map<String, Map<String, Double>> makeSymmetricGraphBasedOnNodes(
		...
		for (String source : graph.keySet()) {
			for (String target : graph.keySet()) {
				if (source.equals(target))
					continue; // 自己ループは考慮しない

				// 両方向のエッジの重みを取得
				double weight = 0.0;
				double revWeight = 0.0;

				if (graph.get(source) != null && graph.get(source).get(target) != null) {
					weight = graph.get(source).get(target);
				}
				if (graph.get(target) != null && graph.get(target).get(source) != null) {
					revWeight = graph.get(target).get(source);
				}
				...
				// 対称な重みを設定
				double symmetricWeight = weight + revWeight;
				if (symmetricWeight > 0.0) {
					result.get(source).put(target, symmetricWeight);
					result.get(target).put(source, symmetricWeight);
				}
			}
		}
