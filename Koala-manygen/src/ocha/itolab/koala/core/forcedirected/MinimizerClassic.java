...
public class MinimizerClassic {
	...
	public void minimizeEnergy(final Map<FdNode,double[]> positions, final int nrIterations) {
		...
		for (FdNode node : nodes) energySum += getEnergy(node);
	 ...
	 */
	private double getRepulsionEnergy(final FdNode node) {
	 ...
	 * @return total energy of the specified node
 	 */
    private double getEnergy(final FdNode node) {
	 ...
	 */
	private double addRepulsionDir(final FdNode node, final double[] dir) {
        if (node.weight == 0.0) return 0.0;
        final double[] position = positions.get(node);
		double dir2 = 0.0;
		for (FdNode node2 : nodes) {
	 ...
	 *               (output parameter)
	 */
    private void getDirection(final FdNode node, final double[] dir) {
	...
	private void printStatistics() {
		...
		for (FdNode node : nodes) meanRepuEnergy += getRepulsionEnergy(node);
