...
def setup_data(timestamps):
    """
    NBAF_coauthorsのデータを加工し、必要なファイル群を作成する。
    """
    print("=========== NBAF_coauthors: setup_data ===========")
...
def __write_filtered_coms(timestamp, alive_nodes):
    ...
    ntwk/配下のデータを参照して、そのtimestampで生きているノードだけを抽出し、結果をcsvファイルとして出力。
    """
    com_fname = f"{DATA_DIR_PATH}coms/runDynamicModularity_NBAF_coauthors_com_{timestamp}_nodes.csv"
    filtered_com_fname = f"{DATA_DIR_PATH}filtered_coms/runDynamicModularity_NBAF_coauthors_com_{timestamp}_nodes.csv"
...
if __name__ == "__main__":
    ...
    timestamp_labels = [year for year in range(2002, 2014)]

    plt.plot(timestamp_labels, density_list)
    plt.xlabel("Timestamp")
    plt.ylabel("Density")
    plt.title("Density of Dataset1 Graph")
    plt.savefig(f"nbaf_coauthors_density_list.png")
