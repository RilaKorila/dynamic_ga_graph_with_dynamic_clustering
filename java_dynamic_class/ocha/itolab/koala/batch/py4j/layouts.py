...
def __scale_size(raw_weights, min_val, max_val):
    ...
    if log_weights:  # リストが空でない場合
        ...
        edge_weights = [min_val + max_val * (w - min_w) / (max_w - min_w) if max_w != min_w else 10 for w in log_weights]
    else:
        edge_weights = []
    return edge_weights
...
def __parallel_move_layout(pos):
    ...
    if min_x < 0 or min_y < 0:
        for node_id in pos.keys():
            ...
            moved_pos[node_id] = (pos[node_id][0] + abs(min_x), pos[node_id][1] + abs(min_y))
...
def get_sammarized_graph(G_super, communities):
    ...
    for community_id, community in enumerate(communities):
        ...
        for node in community:
            for source, target in G_super.edges(node): # nodeが持つedgeは （source, target）と表現
                ...
                metaedge_data = summarized_graph.get_edge_data(community_id_of_source, community_id_of_target)
                ...
                summarized_graph.add_edge(community_id_of_source,  community_id_of_target, weight=new_meta_edge_weight)
