...
def draw_box_plot(df, score_columns, dataset_name, metric, color_start, title_head="Relative Comparison"):
    # 検定結果を格納する辞書
    p_values = {}
    ...
    df_long = df.melt(id_vars='Participants', value_vars=score_columns,
                    var_name='Layout', value_name='Score')
...
if __name__ == "__main__":
    # CSVの読み込み
    df = pd.read_csv('/Users/ayana/vis/dynamic_ga_graph_with_dynamic_clustering/java_dynamic_class/ocha/itolab/koala/batch/py4j/cit.csv')
    ...
    score_columns = ['T1_absolute', 'T2_absolute', 'T3_absolute', 'T4_absolute', 'T5_absolute']
    draw_box_plot(df, score_columns, "cit", "time_absolute", 0, "Absolute")

    # CSVの読み込み
    df = pd.read_csv('/Users/ayana/vis/dynamic_ga_graph_with_dynamic_clustering/java_dynamic_class/ocha/itolab/koala/batch/py4j/facebook.csv')
